...
function loadPreferences(): AppPreferences {
  try {
    ...
    if (merged.onboardingComplete) {
      merged.foodPreferencesComplete = true;
    }
    return merged;
  } catch {
...
export function resolvePostAuthOnboardingScreen(prefs: AppPreferences): FlowScreen {
  if (!prefs.foodPreferencesComplete) return "food-preferences";
...
export function useAppState() {
  ...
  const enterPostAuthOnboarding = useCallback(() => {
    const screen = resolvePostAuthOnboardingScreen(preferences);
    setFlowStack([screen]);
  }, [preferences]);
  ...
  const completeFoodPreferences = useCallback(() => {
    setPreferences((prev) => ({ ...prev, foodPreferencesComplete: true }));
    setFlowStack(["weekly-plan-opt-in"]);
  }, []);
