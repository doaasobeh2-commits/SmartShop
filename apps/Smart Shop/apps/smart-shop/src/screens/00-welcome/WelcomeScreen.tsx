...
const FEATURES = [
  { emoji: "🛒", label: "KI-generierte Einkaufslisten" },
  { emoji: "💰", label: "Budget- & Spar-Intelligenz" },
  { emoji: "🐾", label: "Haustier- & Familienverwaltung" },
] as const;

export function WelcomeScreen({ onNavigate }: ScreenNavigationProps = {}) {
  return (
    <PreviewShell screenNumber={1}>
      <AppShell>
        <div className="flex h-full flex-col">
          ...
          <div className="space-y-2 px-5 pb-5 pt-2">
            <Button onClick={() => onNavigate?.("01-onboarding-ai")}>Jetzt starten</Button>
            <LoginLink />
          </div>
        </div>
      </AppShell>
